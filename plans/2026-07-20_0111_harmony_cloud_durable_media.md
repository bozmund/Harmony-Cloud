...
- Sinkronizirati isti korisnički rezultat kao sigurni logički backup: downloads i status, kolekcije, playliste, albume, izvođače, history, lyrics, queue/session i prenosive postavke.
...
- Kreirati `bozmund/Harmony-Platform` kao infrastructure repo bez .NET solutiona. Servisi dijele versionirani OpenAPI ugovor i generirane klijente.
...
- Nginx rute: `/resolver/*` → Resolver; `/cloud/*` → Harmony Cloud.
...
- Potrebno je jednom rezervirati `harmony-api` naziv u DuckDNS-u.
